...
const Chat = ( props ) => {
  ...
  // const userNickname = location.state.nickname;
   const userNickname = state ? state.nickname : "";
   const oauthId = state ? state.oauthId : "";
   const profileImageUrl = state ? state.profileImageUrl : "";
  ...
  useEffect(() => {
    ...
    return () => {
      disconnect(); // 컴포넌트가 언마운트될 때 웹소켓 연결을 해제합니다.
    };
  }, []);
  ...
  const imageProfileStyle = {
    borderRadius: "50%", // 50%로 설정하여 원 모양 테두리 생성
    width: "50px", // 원하는 크기로 설정 (예: 100px)
    height: "50px", // 원하는 크기로 설정 (예: 100px)
  };
  ...
  return (
    ...
    <img src={profileImageUrl} alt="Profile Image" style={imageProfileStyle} />
